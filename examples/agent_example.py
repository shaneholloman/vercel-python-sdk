...
def get_time(timezone: str) -> str:
    ...
    tz_map = {
        "UTC": tz.utc,
        "America/New_York": tz(tz.timedelta(hours=-5)),
        "America/Los_Angeles": tz(tz.timedelta(hours=-8)),
        "Europe/London": tz(tz.timedelta(hours=0)),
        "Asia/Tokyo": tz(tz.timedelta(hours=9))
    }
...
def main():
    ...
    print("-" * 30)
    response = agent.run("What's 25 * 4, and what's the weather like in San Francisco? Also, what time is it in UTC?")
    print(f"Response: {response}")
